...
# Allow direct execution: python scripts/sensitivity.py
...
def report(nodes, edges, tx, cfg, top_n=20):
    ...
    for key in ROLE_THRESHOLDS:
        for shift in (-1, 1):
            ...
            differences = [
                str(gid)
                for gid in base_roles.index
                if base_roles[gid] != current[gid]
            ]
            ...
            thresholds.append(
                {
                    ...
                    "baseline": cfg[key],
                    "value": value,
                    "changed_roles": len(differences),
                    "changed_gids": differences,
                }
            )
    ...
    for key, weight in WEIGHTS.items():
        for factor in (0.8, 1.2):
            # Only one factor changes; rescale to retain
    ...
    return {
        "note": (
            "Сценарии чувствительности, не оценка accuracy "
            "или вероятности виновности."
        ),
        "top_n": count,
        "baseline_top_gids": base_top,
...
def main():
    ...
    parser.add_argument(
        "--out",
        type=Path,
        default=Path("results/sensitivity.json"),
    )
    ...
    print(
        f"Отчёт: {args.out}; изменения ролей: "
